...
export default function Navbar() {
  ...
  return (
    <nav className='text-white py-5 flex justify-between lg:justify-start items-center gap-6 lg:gap-44'>
      ...
      <div className={`z-[1000] w-screen md:hidden bg-primaryBlueSecondary transition-all ease-in-out origin-left px-5 py-6 absolute h-screen top-0 left-0 flex flex-col gap-5 ${open ? "opacity-100 translate-x-0" : "opacity-0 translate-x-96"} duration-300`}>
        <div onClick={toggleMenu} className="cursor-pointer md:hidden">
          <FiX size={30} />
        </div>
